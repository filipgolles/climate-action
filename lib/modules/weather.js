...
async function formatAndAssemble(rawData) {
  ...
  const str =
    `Conditions in ${res.location.name}, ${res.location.country}
    are currently ${res.current.condition}
    with a temperature of ${res.current.temperature}
    degrees. Later today it looks like ${res.later.condition}
    and ${res.later.temperature} degrees.
    Why not ride your bike to work today?`;
  return str;
}
