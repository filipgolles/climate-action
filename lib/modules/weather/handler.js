...
module.exports = async action => {
  console.log(`handle weather job, location: ${ action.userStorage.location }`);
  const r = await fireRequest(action.userStorage.location);
  action.tell(await formatAndAssemble(r.body));
};

module.exports.apixuTest = async () => {
  return await fireRequest('umea');
};

module.exports.assemblyTest = async body => {
  return await formatAndAssemble(body);
};
